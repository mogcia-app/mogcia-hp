...
export default function FloatingContact() {
  ...
  return (
    <>
      {!isOpen && (
        <div className="fixed bottom-4 right-16 z-[1900] sm:bottom-5 sm:right-32 md:bottom-6 md:right-8">
          <button
            type="button"
            onClick={() => setIsOpen(true)}
            className="relative z-0 flex h-20 w-20 items-center justify-center overflow-visible rounded-full bg-white shadow-[0_20px_55px_rgba(24,32,56,0.3)] transition-transform duration-300 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-gray-900/20"
            aria-expanded={isOpen}
            aria-controls="floating-contact-panel"
          >
            <span className="pointer-events-none absolute -inset-4 -z-10 rounded-full bg-gradient-to-br from-[#f6e27a]/40 via-[#d2c7ff]/30 to-transparent blur-2xl opacity-80" />
            <img src="/1M%20Logo.png" alt="お問い合わせ" className="relative h-14 w-14 -translate-y-[2px] object-contain" />
          </button>
          <div className="mt-2 text-[10px] uppercase tracking-[0.35em] text-gray-400">Ask</div>
        </div>
      )}
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex flex-col items-end gap-2">
            <button
              ...
              className="rounded border border-gray-200 px-2 py-[2px] text-[10px] text-gray-500 transition-colors hover:border-gray-900 hover:text-gray-900"
            >
